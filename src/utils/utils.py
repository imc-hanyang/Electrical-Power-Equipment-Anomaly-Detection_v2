...
def load_ckpt_finetune(path, model, optimizer=None, logger=None, args=None):
    ...
    allow_new_rank_head = getattr(args, 'pooling_mode', None) == 'rank_mil'
    incompat = model.load_state_dict(dicts['state_dict'], strict=not allow_new_rank_head)
    if allow_new_rank_head:
        allowed_missing = {
            'rank_mil_norm.weight',
            'rank_mil_norm.bias',
            'rank_mil_head.weight',
            'rank_mil_head.bias',
        }
        unexpected_missing = set(incompat.missing_keys) - allowed_missing
        if unexpected_missing or incompat.unexpected_keys:
            raise RuntimeError(
                f'Unexpected checkpoint mismatch: missing={sorted(unexpected_missing)}, '
                f'unexpected={incompat.unexpected_keys}'
            )
        with torch.no_grad():
            model.rank_mil_head.weight.copy_(model.head.weight)
            model.rank_mil_head.bias.copy_(model.head.bias)
